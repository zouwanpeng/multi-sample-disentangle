...
def convert_adj(sparse_mat):
    ...
    # 避免 torch.from_numpy：部分环境下 PyTorch 报 "Numpy is not available"
    rowcol = np.stack((sparse_mat.row, sparse_mat.col), axis=0).astype(np.int64)
    indices = torch.tensor(rowcol, dtype=torch.long)
    values = torch.tensor(sparse_mat.data, dtype=torch.float32)
